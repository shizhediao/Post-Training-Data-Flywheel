...
async def process_api_requests_from_list(
    ...
    # with open(requests_filepath) as file:
    # `requests` will provide requests one at a time
    requests = requests_list.__iter__()
    logging.debug(f"File opened. Entering main loop")
    async with aiohttp.ClientSession() as session:  # Initialize ClientSession here
        while True:
            # get next request (if one is not already waiting for capacity)
...
def api_requests_from_list(
    ...
    return asyncio.run(process_api_requests_from_list(
        requests_filepath=None,
        save_filepath=None,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_requests_per_minute", type=int, default=5_000)
    parser.add_argument("--max_tokens_per_minute", type=int, default=2_000_000)
    parser.add_argument("--token_encoding_name", default="cl100k_base")
    parser.add_argument("--max_attempts", type=int, default=5)
    parser.add_argument("--logging_level", default=logging.WARNING)
